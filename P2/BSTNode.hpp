...
public:
  ...
  BSTNode<Data>* successor() { 
    ...
    if(currNode->right != NULL) { // if right is not null
       currNode = currNode->right; // right is currNode
      while(currNode->left != NULL) { // while left is not null
        currNode = currNode->left; // left is currNode
      }
      return currNode; // return currNode
    }
    else if (parent == NULL) { // if parent is null
      return NULL; // return null
    } 
    else if (currNode == parent->left) { // if currNode is the left node
      return parent; // return parent
    } 
    else {
      return parent->previousParent(); // return right nodes successor
    }
  }

  BSTNode<Data>* previousParent() {
    /* Find and return a successor that is an ancestor, if it exists. */
    if (parent==NULL) {
      return NULL;
    } else if (this == parent->right) {
      return parent->previousParent();
    } else {
      return parent;
    }
  }
